...
export default function Result() {
  ...
  const fetchAudit = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://spendlens-7nbz.onrender.com'}/api/audit/${id}`);
      if (res.ok) {
  ...
  const fetchSummary = async (data) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://spendlens-7nbz.onrender.com'}/api/audit/summary`, {
        method: 'POST',
  ...
  const handleCapture = (e) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'https://spendlens-7nbz.onrender.com'}/api/leads`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
        email, 
